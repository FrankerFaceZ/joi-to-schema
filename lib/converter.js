 ...
 * @param {String} [opts.extractPath] If this value is set, {@link Converter#convert}
 * will extract sub-trees from the generated schema and store them on
 ...
 *
 * If a Joi object has attached `className` metadata, and the {@link Converter}
 * instance has a set `extractPath` option,  the resulting schema will be
...
Converter.prototype._convert = function(thing) {
	...
	if ( thing.flags.presence ) {
		const meta = out.meta = out.meta || {};
		meta.presence = thing.flags.presence;
	}
	...
	if ( this.opts.extractPath && out.meta && out.meta.className ) {
