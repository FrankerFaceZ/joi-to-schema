...
Converter.prototype.convert = function(schema) {
	if ( ! schema.isJoi && ! (schema['$_root'] && schema['$_root'].isSchema && schema['$_root'].isSchema(schema)) )
...
Converter.prototype._convert = function(thing) {
	...
	if ( thing.meta )
		...
		out.meta = Object.assign({}, ...thing.metas);
	...
	} else if ( thing.flags.unit ) {
		const meta = out.meta = out.meta || {};
		meta.unit = thing.flags.unit;
	}
	...
	else if ( thing.flags.label )
		out.title = thing.flags.label;

	if ( thing.description )
		out.description = thing.description;
	else if ( thing.flags.description )
		out.description = thing.flags.description;
